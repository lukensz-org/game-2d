...
std::vector<std::uint32_t> loadSpirv(const char *path) {
  ...
  if (!file) {
    throw std::runtime_error(std::string("Could not open generated SPIR-V: ") +
                             path);
  }
  const std::streamsize byte_size = file.tellg();
  if (byte_size <= 0 ||
      (byte_size % static_cast<std::streamsize>(sizeof(std::uint32_t))) != 0) {
    throw std::runtime_error(
        std::string("Generated SPIR-V has invalid byte size: ") + path);
  }
  file.seekg(0);
  std::vector<std::uint32_t> words(static_cast<std::size_t>(byte_size) /
                                   sizeof(std::uint32_t));
  if (!file.read(reinterpret_cast<char *>(words.data()), byte_size)) {
    throw std::runtime_error(std::string("Could not read generated SPIR-V: ") +
                             path);
  }
  return words;
}
  ...
  try {
    ...
    while (!window.shouldClose()) {
      ...
      if (options.bounded && ++bounded_iterations > bounded_iteration_limit) {
        throw std::runtime_error(
            "Bounded rendering did not complete its presentation "
      ...
      case FrameBeginDisposition::surface_lost:
        throw std::runtime_error(
            "Presentation surface was lost while beginning a frame.");
      ...
      for (std::size_t index = 0; index < kSquares.size(); ++index) {
        ...
        draws[index] = DrawInfo{
            ...
            .push_constants =
                std::span<const PushConstantWrite>{&push_writes[index], 1},
            .vertex_count = 6,
            .instance_count = 1,
        };
      }
